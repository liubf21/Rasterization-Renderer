...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    int xmax = std::max(std::max(t.v[0].x(), t.v[1].x()), t.v[2].x()) + 1;
    int xmin = std::min(std::min(t.v[0].x(), t.v[1].x()), t.v[2].x()) - 1;
    int ymax = std::max(std::max(t.v[0].y(), t.v[1].y()), t.v[2].y()) + 1;
    int ymin = std::min(std::min(t.v[0].y(), t.v[1].y()), t.v[2].y()) - 1;
        ...
        for (int y = ymin; y <= ymax; y++)
        {
            if (insideTriangle(x, y, t.v))
            {
                ...
                if (z_interpolated < depth_buf[get_index(x, y)])
                {
                    ...
                    auto pixel_color = fragment_shader(payload); // fragment_shader is a function pointer passed in from main.cpp
